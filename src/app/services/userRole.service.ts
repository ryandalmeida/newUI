...
export class UserRoleService {
  ...
  constructor(private http: HttpClient, private msAdalSvc: MsAdalAngular6Service) { }
   ...
   getRole() {
    ...
    return new Promise((resolve, reject) => {
      this.http.post<any>('http://10.177.197.134:9090/userRole/searchUserRole', myObject, this.headers)
        .subscribe(response => {
  ...
  searchUserRole(myPostObject) {
    return this.http.post<any>('http://10.103.42.177:9090/userRole/searchUserName', myPostObject, { headers: new HttpHeaders().set('Azure-Auth', 'true') })
  }

  updateUserRole(): Observable<UserRoleData[]> {
    return this.http.get<UserRoleData[]>('http://10.103.42.177:9090/userRole/updateUserRole');
  }
  
  searchUserName(emailId){
    return this.http.post<any>('http://10.177.197.134:9090/userRole/searchUserName', emailId, this.headers)
  } 
