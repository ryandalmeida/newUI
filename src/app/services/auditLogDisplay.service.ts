...
export class AuditLogDisplayService {
  ...
  searchAuditLogData(myPostObject) {
    return this.http.post<any>('http://10.103.42.177:8086/auditAzure/searchAuditLogData', myPostObject, { headers: new HttpHeaders().set('Authorization', `Bearer ${this.authtoken}`).set('ROLES', `${this.roleObject}`).set('uniqueName',`${this.authenticationService.userInfo.profile.unique_name}`)});
  }
