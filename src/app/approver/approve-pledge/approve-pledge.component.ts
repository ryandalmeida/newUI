...
export class ApprovePledgeComponent implements OnInit {
  ...
  onRevise() {
    ...
    this.pledgeService.revisePledge(this.reviseArray).subscribe(response => {
      ...
      if(response === "SUCCESS"){
        var dialogData = "Success: Pledge revised successfully."
      }
      if(response === "FAILURE"){
        var dialogData = "Failed: Pledge revision failed."
      }
      this.spinnerService.hide();
  ...
  approvePledge() {
   // setTimeout(() => {
      ...
      this.pledgeService.approvePledge(myPostObject).subscribe(response => {
        console.log(response);
        if(response === "SUCCESS"){
          var dialogData = "Success: Pledge approved successfully."
        }
        if(response === "FAILURE"){
          var dialogData = "Failed: Pledge failed to be approved."
        }
        this.spinnerService.hide();
