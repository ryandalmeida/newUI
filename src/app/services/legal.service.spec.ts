import { TestBed, inject } from '@angular/core/testing';
import { HttpEvent, HttpEventType, HttpClient } from '@angular/common/http';
import { Adal6HTTPService, Adal6Service } from 'adal-angular6';
import {
  HttpClientTestingModule,
  HttpTestingController
} from '@angular/common/http/testing';
import { LegalService } from './legal.service';
import { LegalData } from '../models/legal.model';
import { PledgeData } from '../models/pledge.model';

describe('LegalService', () => {
  let httpClient: HttpClient;

  let donorLegalTableTestData: Array<PledgeData> = [
    {
      pledgeId: 20,
      programName: 'Carbon Emission Reduction',
      pledgeFundType: 'Regular', 
      amount: 500000000,
      installments: 10,
      paymentPeriod: 10,
      startDate: new Date('2017-09-03'),
      endDate: new Date('2018-09-04'),
      country:'India',
      status:'New'
    }
  ];

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      providers: [LegalService, Adal6Service, {
        provide: Adal6HTTPService,
        useFactory: Adal6HTTPService.factory,
        deps: [Adal6Service]
      }]
    });
  });

  beforeEach(() => {
    httpClient = TestBed.get(HttpClient);
  });

  it(
    'should get users with status "Legal Recorded by donor"',
    inject(
      [HttpTestingController, LegalService],
      (httpMock: HttpTestingController, dataService: LegalService) => {

        dataService.getAllLegalRecordedbyDonor().subscribe((res) => {
          console.log("res", res)
          expect(res).toEqual(donorLegalTableTestData)
          expect(res.length).toEqual(1);

        });

        const mockReq = httpMock.expectOne(dataService.getLegalDonorUrl);
        expect(mockReq.request.method).toEqual('GET');
        mockReq.flush(donorLegalTableTestData);
        httpMock.verify();
      }
    )
  );



  it(
    'should get users with status "Approved"',
    inject(
      [HttpTestingController, LegalService],
      (httpMock: HttpTestingController, dataService: LegalService) => {

        dataService.getAllLegalrecords().subscribe((res) => {
          console.log("res", res)
          expect(res).toEqual(donorLegalTableTestData)
          expect(res.length).toEqual(1);

        });

        const mockReq = httpMock.expectOne(dataService.getApprovedUrl);
        expect(mockReq.request.method).toEqual('GET');
        mockReq.flush(donorLegalTableTestData);
        httpMock.verify();
      }
    )
  );

  it('submit donor details',
    inject([HttpTestingController, LegalService],
      (httpMock: HttpTestingController, service: LegalService) => {

        const mockBody =
        {
          pledgeId: 7,
          donorName: "x",
          wbgBusinessUser: "abc",
          donorSign: "bxxxxbbb"
        }
        service.onLegalSubmitDonor(mockBody).subscribe((data: Object) => {
          console.log("data..", data);
          expect("Legal Agreement data has been inserted Successfully").toEqual("Legal Agreement data has been inserted Successfully")
          //expect(data).toBe(21)
        });
        const mockReq = httpMock.expectOne("https://5ufe1v6q92.execute-api.us-east-1.amazonaws.com/test/tasktokenpoller");
        expect(mockReq.request.method).toEqual('POST');
        mockReq.flush(donorLegalTableTestData);

        httpMock.verify();
      }));

  it('submit approver details',
    inject([HttpTestingController, LegalService],
      (httpMock: HttpTestingController, service: LegalService) => {

        const mockBody =
        {
          pledgeId: 7,
          wbgBusinessUserSign: "bxxxxbbb"
        }
        service.onLegalSubmitWBGUser(mockBody).subscribe((data: Object) => {
          console.log("data..", data);
          expect("Legal Agreement data has been inserted Successfully").toEqual("Legal Agreement data has been inserted Successfully")

        });
        const mockReq = httpMock.expectOne("https://5ufe1v6q92.execute-api.us-east-1.amazonaws.com/test/tasktokenpoller");
        expect(mockReq.request.method).toEqual('POST');
        mockReq.flush(donorLegalTableTestData);
        httpMock.verify();
      }));


  let donorSignTableTestData = [
    {
      donorSign: "data:image/png;base64,"
    },
  ];

  it('get donor sign',
    inject([HttpTestingController, LegalService],
      (httpMock: HttpTestingController, service: LegalService) => {

        const mockBody =
        {
          pledgeId: 7,

        }
        service.getSign(mockBody).subscribe((data: Object) => {
          console.log("data..", data);
          expect("data:image/png;base64,")
            .toEqual("data:image/png;base64,")
          //expect(data).toBe(21)
        });
        const mockReq = httpMock.expectOne("https://wbg-bpm-apim.azure-api.net/legal-api/legalAgreement/getSign");
        expect(mockReq.request.method).toEqual('POST');
        mockReq.flush(donorSignTableTestData);
        httpMock.verify();
      }));

});