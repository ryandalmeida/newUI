...
export class GenerateBillingComponent implements OnInit {
  ...
  generateBilling() {
    ...
    this.http.post('https://5ufe1v6q92.execute-api.us-east-1.amazonaws.com/test/generatebillandinvoice-generatebill', this.checkedArray, { headers: new HttpHeaders().set('aws-auth', 'true') })
      .subscribe(response => {
        console.log(response);
        if (response === "SUCCESS") {
