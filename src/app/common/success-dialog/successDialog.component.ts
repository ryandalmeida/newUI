...
export class SuccessDialogComponent {
  ...
  onClick() {
    ...
    if(((this.data.header).toString()).includes("Account Received")){
      this.http.get('https://wbgeabpmservicesaz.worldbankgroup.org/pledge/updateAr', { headers: new HttpHeaders().set('Authorization', `Bearer ${this.authtoken}`).set('ROLES', `${this.roleObject}`).set('uniqueName',`${this.authenticationService.userInfo.profile.unique_name}`)});
    }
    this.dialogRef.close();
  }
}
