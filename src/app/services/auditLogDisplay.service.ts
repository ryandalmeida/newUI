...
export class AuditLogDisplayService {
  ...
  searchAuditLogData(myPostObject) {
    return this.http.post<any>('http://10.103.42.177:8085/auditAzure/searchAuditLogData', myPostObject, { headers: new HttpHeaders().set('Authorization', `Bearer ${this.authtoken}`).set('ROLES', `${this.roleObject}`).set('uniqueName',`${this.authenticationService.userInfo.profile.unique_name}`)});
  }
  ...
  getAllAuditAzure(): Observable<any[]> {
    return this.http.get<AuditLogData[]>('http://10.103.42.177:8086/auditAzure/getAuditAzure', { headers: new HttpHeaders().set('Authorization', `Bearer ${this.authtoken}`).set('ROLES', 'ADMIN').set('uniqueName','NEEMA')});
  }
}
