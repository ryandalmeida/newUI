...
export class AuditLogDisplayService {
  ...
  searchAuditLogData(myPostObject) {
    return this.http.post<any>(this.audit_aws_url+'/auditAWS/searchAuditLogDataAWS', myPostObject, { headers: new HttpHeaders().set('Authorization', `Bearer ${this.authtoken}`).set('ROLES', `${this.roleObject}`).set('uniqueName',`${this.authenticationService.userInfo.profile.unique_name}`)});
  }
